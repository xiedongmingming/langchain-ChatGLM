    ...
    if mode == "知识库问答" and os.path.exists(vs_path):
        ...
        if os.path.exists(vs_path):
            ...
            if not resp["source_documents"]:
                ...
                yield history + [[query, "根据您的设定，没有匹配到任何内容，请确认您设置的知识相关度 Score 阈值是否过小或其他参数是否正确。"]], ""
...
def change_vs_name_input(vs_id, history):
    ...
    if vs_id == "新建知识库":
        ...
        return gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), os.path.join(
            VS_ROOT_PATH,
            vs_id
