...
def add_vs_name(vs_name, chatbot):
    ...
    if vs_name in get_vs_list():
        ...
        if not os.path.exists(os.path.join(UPLOAD_ROOT_PATH, vs_name)):  # 新建上传文件存储路径
            #
            os.makedirs(os.path.join(UPLOAD_ROOT_PATH, vs_name))

        if not os.path.exists(os.path.join(VS_ROOT_PATH, vs_name)):  # 新建向量库存储路径
            #
...
with gr.Blocks(css=block_css, theme=gr.themes.Default(**default_theme_args)) as demo:
    ...
    vs_path, file_status, model_status = gr.State(
        os.path.join(VS_ROOT_PATH, get_vs_list()[0]) if len(get_vs_list()) > 1 else ""
    ), gr.State(""), gr.State(model_status)
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with vs_setting:
                    ...
                    vs_name = gr.Textbox(
                        label="请输入新建知识库名称，当前知识库命名暂不支持中文",
                        lines=1,
                        interactive=True,
                        visible=True
                    )
                    ...
                    with file2vs:
                        ...
                        sentence_size = gr.Number(
                            value=SENTENCE_SIZE, precision=0,
                            label="文本入库分句长度限制",
        ...
        with gr.Row():
            ...
            with gr.Column(scale=5):
                ...
                with knowledge_set:
                    ...
                    chunk_conent = gr.Checkbox(
                        value=False,
                        label="是否启用上下文关联",
                        interactive=True
                    )

                    chunk_sizes = gr.Number(
                        value=CHUNK_SIZE,
                        precision=0,
                ...
                with vs_setting:
                    ...
                    vs_refresh.click(  # 将上传的文件保存到CONTENT文件夹下，并更新下拉框
                        fn=refresh_vs_list,
                        inputs=[],
                    ...
                    load_folder_button.click(
                        get_vector_store,
                        show_progress=True,
                        inputs=[select_vs_test, folder_files, sentence_size, chatbot, vs_add, vs_add],
                        outputs=[vs_path, folder_files, chatbot],
                    )
    ...
    with gr.Tab("模型配置"):
        ...
        no_remote_model = gr.Checkbox(
            shared.LoaderCheckPoint.no_remote_model,
            label="加载本地模型",
            interactive=True
        )
        ...
        load_model_button.click(
            ...
            show_progress=True,
            inputs=[
                llm_model, embedding_model, llm_history_len, no_remote_model, use_ptuning_v2, use_lora, top_k, chatbot
            ],
            outputs=chatbot
        )
        load_knowlege_button = gr.Button("重新构建知识库")
        load_knowlege_button.click(
            reinit_vector_store,
            show_progress=True,
            inputs=[select_vs, chatbot],
            outputs=chatbot
