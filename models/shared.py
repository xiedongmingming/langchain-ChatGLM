...
def loaderLLM(llm_model: str = None, no_remote_model: bool = False, use_ptuning_v2: bool = False) -> Any:
    ...
    if loaderCheckPoint.no_remote_model:
        loaderCheckPoint.model_name = llm_model_info['name']  # 本地
    else:
        loaderCheckPoint.model_name = llm_model_info['pretrained_model_name']  # 远端
    ...
    elif 'ChatGPT' in llm_model_info["provides"]:
        pass
    else:
        loaderCheckPoint.reload_model()

    provides_class = getattr(sys.modules['models'], llm_model_info['provides'])  # LLM实现类

    modelInsLLM = provides_class(checkPoint=loaderCheckPoint)  # 实例化GLM
